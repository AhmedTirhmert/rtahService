...
class ServiceController extends BaseController
{
    ...
    public function index()
    {
        if (Auth::user()->isAdmin()) {
            $Services = $this->Service->with('user', 'product', 'request', 'request.feedback.client', 'request.complain','request.complain.client', 'product.category')->latest()->paginate(10);
        } else {
            $Services = $this->Service->with('user', 'product', 'request', 'request.feedback.client', 'request.complain','request.complain.client', 'product.category')->where('user_id', '!=', Auth::user()->id)->latest()->paginate(10);
        }
    ...
    {
        if (Auth::user()->isAdmin()) {
            $Services = $this->Service->with('user', 'product', 'request', 'request.feedback', 'request.complain','request.complain.client', 'product.category')->latest()->paginate(10);
        } else {
            $Services = $this->Service->with('user', 'product', 'request', 'request.feedback', 'request.complain','request.complain.client', 'product.category')->where('user_id', Auth::user()->id)->latest()->paginate(10);
        }
        return $this->sendResponse($Services, 'Service list');
    }
